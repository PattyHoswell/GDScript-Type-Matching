...
func _check_csharp_script(from_resource: bool) -> void:
	if Type.inherit_from("TestChildCSharp", "TestParentCSharp"):
		print("TestChildCSharp inherit TestParentCSharp")
	
	if from_resource:
		print(Type.extending_from(ResourceLoader.load("res://TestChildCSharp.cs"), true))
	else:
		print(Type.extending_from(get_node("TestChildCSharp").get_script(), true))
...
func _array_pattern_match_v2() -> void:
	...
	match extending_types:
		...
		_ when extending_types.has(Node3D):
